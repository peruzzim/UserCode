...
void fit_dataset(const char* datafilename, const char* sigfilename, const char* bkgfilename, TString varname, TString splitting, float leftrange, float rightrange){
...

  {
    ...
    if (splitting=="EBEB") {
      ...
      sigfile->GetObject(helper.get_roohist_name(varname,TString("sig"),TString("EB"),TString("1")),roohist[0][0]);
      sigfile->GetObject(helper.get_roohist_name(varname,TString("sig"),TString("EB"),TString("2")),roohist[0][1]);
      bkgfile->GetObject(helper.get_roohist_name(varname,TString("bkg"),TString("EB"),TString("1")),roohist[1][0]);
      bkgfile->GetObject(helper.get_roohist_name(varname,TString("bkg"),TString("EB"),TString("2")),roohist[1][1]);
    }
    else if (splitting="EBEE"){
      datafile->GetObject(helper.get_roovar_name(varname,0,0),roovar[0]);
      datafile->GetObject(helper.get_roovar_name(varname,1,1),roovar[1]);
      sigfile->GetObject(helper.get_roohist_name(varname,TString("sig"),TString("EB"),TString("1")),roohist[0][0]);
      sigfile->GetObject(helper.get_roohist_name(varname,TString("sig"),TString("EE"),TString("2")),roohist[0][1]);
      bkgfile->GetObject(helper.get_roohist_name(varname,TString("bkg"),TString("EB"),TString("1")),roohist[1][0]);
      bkgfile->GetObject(helper.get_roohist_name(varname,TString("bkg"),TString("EE"),TString("2")),roohist[1][1]);
    }
    else if (splitting="EEEE"){
      datafile->GetObject(helper.get_roovar_name(varname,1,0),roovar[0]);
      datafile->GetObject(helper.get_roovar_name(varname,1,1),roovar[1]);
      sigfile->GetObject(helper.get_roohist_name(varname,TString("sig"),TString("EE"),TString("1")),roohist[0][0]);
      sigfile->GetObject(helper.get_roohist_name(varname,TString("sig"),TString("EE"),TString("2")),roohist[0][1]);
      bkgfile->GetObject(helper.get_roohist_name(varname,TString("bkg"),TString("EE"),TString("1")),roohist[1][0]);
      bkgfile->GetObject(helper.get_roohist_name(varname,TString("bkg"),TString("EE"),TString("2")),roohist[1][1]);
    }

    datafile->GetObject(helper.get_roodset_name(varname,splitting),roodset);
  ...
  for (int i=0; i<2; i++){
    ...
    if (i==0){
    sig1pdf.plotOn(varframe[i],Normalization(nsigsig.getVal()+nsigbkg.getVal(),RooAbsReal::NumEvent));
    bkg1pdf.plotOn(varframe[i],Normalization(nbkgbkg.getVal()+nbkgsig.getVal(),RooAbsReal::NumEvent));
    }
    else if (i==1){
    sig2pdf.plotOn(varframe[i],Normalization(nsigsig.getVal()+nbkgsig.getVal(),RooAbsReal::NumEvent));
    bkg2pdf.plotOn(varframe[i],Normalization(nbkgbkg.getVal()+nsigbkg.getVal(),RooAbsReal::NumEvent));
    }
