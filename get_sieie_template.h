...
#ifndef get_sieie_template_h
#define get_sieie_template_h
...
public :
   ...
   get_sieie_template(TTree *tree=0);
   virtual ~get_sieie_template();
   ...
   Int_t Cut_egm_10_006_sieierelaxed(Long64_t entry);
   Int_t Cut_egm_10_006_sieierelaxed_sideband(Long64_t entry);
   void     WriteOutput(const char* filename);
   void     ShowHistogram(int i);
   void     SetParams(bool _isdata, bool _sideband, bool _barrel);

   TRandom3 *randomgen;
   RooRealVar *sieievar;
   RooDataHist *hsieie[3];
...
#ifdef get_sieie_template_cxx
get_sieie_template::get_sieie_template(TTree *tree)
{
...
// used to generate this class and read the Tree.
   ...
   sieievar = new RooRealVar("sieievar","sieievar",0,0.05);
   sieievar->setBins(50);
   
   randomgen = new TRandom3(0);
   hsieie[0] = new RooDataHist("sieie_bkg","sieie_bkg",*sieievar);
   hsieie[1] = new RooDataHist("sieie_sig","sieie_sig",*sieievar);
   hsieie[2] = new RooDataHist("sieie_all","sieie_all",*sieievar);

}

get_sieie_template::~get_sieie_template()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
   delete randomgen;
   delete sieievar;
   for (int i=0;i<3;i++) delete hsieie[i];
}

Int_t get_sieie_template::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t get_sieie_template::LoadTree(Long64_t entry)
{
...
void get_sieie_template::Init(TTree *tree)
{
...
Bool_t get_sieie_template::Notify()
{
...
void get_sieie_template::Show(Long64_t entry)
{
...
Int_t get_sieie_template::Cut(Long64_t entry)
{
...
Int_t get_sieie_template::Cut_egm_10_006_sieierelaxed(Long64_t entry)
{
// This function may be called from Loop.
// returns  1 if entry is accepted.
// returns -1 otherwise.
...
// egm10006 loose, sietaieta relaxed
  ...
  /* std::cout << "bla " << pholead_PhoIso04TrkHollow << std::endl; */

  if (pholead_PhoHasPixSeed) return -1; 
  ...
  /* std::cout << "pholead passed" << std::endl; */

  if (photrail_PhoHasPixSeed) return -1;
...
Int_t get_sieie_template::Cut_egm_10_006_sieierelaxed_sideband(Long64_t entry)
{
// This function may be called from Loop.
// returns  1 if entry is accepted.
// returns -1 otherwise.

// egm10006 loose, sietaieta relaxed in sideband
...
void get_sieie_template::WriteOutput(const char* filename){
...
void get_sieie_template::ShowHistogram(int i){
  RooPlot *sieievarframe = sieievar->frame(Name("sietaieta"),Title("sietaieta"));
  hsieie[i]->plotOn(sieievarframe);
  sieievarframe->Draw();
};
void get_sieie_template::SetParams(bool _isdata, bool _sideband, bool _barrel){
  isdata=_isdata;
  sideband=_sideband;
  barrel=_barrel;
};
#endif // #ifdef get_sieie_template_cxx
