...
void plot_datasets_axis1(std::vector<RooDataSet*> dset, TString outname, legend_struct legdata);
...
fit_output* fit_dataset(const char* inputfilename_t2p, const char* inputfilename_t1p1f, const char* inputfilename_t2f, const char* inputfilename_d, TString diffvariable, TString splitting, int bin, const TString do_syst_string=TString("")){
  ...
  if (do_syst_string=="templateshapeMCtrue" || do_syst_string=="savepdfMCtrue"){
  ...
  templatebinsboundaries = (islowstatcat) ? templatebinsboundaries_reduced+0 : templatebinsboundaries_full+0;
  Double_t templatebinsboundaries_diagonal[n_templatebins_const+1];
...
//  RooDataSet *dataset =        (RooDataSet*)(dataset_orig->Clone("dataset"));
  ...
  */
  ...
  if (doxcheckstemplates) {
    ...
    plot_datasets_axis1(sigdsets,Form("plots/histo_sig_%s.root",s1.Data()),sigleg);

    legend_struct sigbkg;
    sigbkg.title = Form("Background template %s",s1.Data());
    sigbkg.leg.push_back(TString("Sieie sideband in data"));
    sigbkg.leg.push_back(TString("Photon Iso in MC fakes"));
    sigbkg.leg.push_back(TString("Sieie sideband in MC"));
    ...
    plot_datasets_axis1(bkgdsets,Form("plots/histo_bkg_%s.root",s1.Data()),sigbkg);
  ...
  RooHistPdf *sigsigpdf_forgen = NULL;
  RooHistPdf *sigbkgpdf_forgen = NULL;
  RooHistPdf *bkgsigpdf_forgen = NULL;
  RooHistPdf *bkgbkgpdf_forgen = NULL;
  ...
  for (int runcount=0; runcount<times_to_run; runcount++){
	...
	dataset          =(RooDataSet*)(original_dataset->Clone("dataset_forsyst"));
      ...
      if (do_syst_string==TString("templatestatistics") || do_syst_string==TString("templateshapeMCtrue") || do_syst_string==TString("templateshapeMCpromptdriven") || do_syst_string==TString("templateshapeMCfakedriven")){
...
//    if (doplots) {
//      plot_template_dependency_axis1(dataset_bkg_axis1,TString("pt"),20,70,5,kTRUE);
...
//    pp->setRange(minpp,maxpp);
    ...
    */
    ...
    if (doplots){
      ...
      model_axis2_unbinned->plotOn(frame2bla,Components("sigpdf_axis2_unbinned"),Normalization(fbkgsig->getVal()/fsig2->getVal(),RooAbsPdf::Relative),LineStyle(kDashed),LineColor(kGreen));
...
//      h3h->Draw("same");
    ...
    if (do_syst_string==TString("savepdfMCtrue")){
  ...
  if (writeoutpurity){
    ...
    for (int i=0; i<4; i++){
      ...
      if (do_syst_string==TString("templateshapeMCtrue") || do_syst_string==TString("templateshapeMCpromptdriven") || do_syst_string==TString("templateshapeMCfakedriven") || do_syst_string==TString("templateshapeMCfulldriven")) {name+=do_syst_string; name.Append("_");}
    ...
    TString helper("");
    if (do_syst_string==TString("templateshapeMCtrue") || do_syst_string==TString("templateshapeMCpromptdriven") || do_syst_string==TString("templateshapeMCfakedriven") || do_syst_string==TString("templateshapeMCfulldriven")) {helper = do_syst_string; helper+=TString("_");}
...
void post_process(TString diffvariable="", TString splitting="", bool skipsystematics=false){
  ...
  TString xunit;
  if (diffvariable==TString("invmass") || diffvariable==TString("diphotonpt")) xunit=TString("GeV");
  if (diffvariable==TString("costhetastar") || diffvariable==TString("dphi")) xunit=TString("1");
  ...
  if (!skipsystematics){
    ...
    xsec_ngammagammayield = (TH1F*)(xsec->Clone("xsec_rawngammagammayield"));
  ...
  TLegend *leg = new TLegend(0.7,0.7,0.9,0.9,Form("purity %s",splitting.Data()));
  leg->AddEntry(purity[0],"prompt-prompt","lp");
  leg->AddEntry(purity[1],"prompt-fake","lp");
  if (!sym) leg->AddEntry(purity[2],"fake-prompt","lp");
  ...
  xsec_withsyst->SetTitle(Form("Cross section %s",splitting.Data()));
  xsec_withsyst->GetYaxis()->SetTitle(Form("d#sigma /fb /%s",xunit.Data()));
  xsec_withsyst->SetMinimum(0);
  xsec_withsyst->Draw("e1");
  xsec->Draw("e1same");

  TLegend *legxsec = new TLegend(0.7,0.7,0.9,0.9);
  legxsec->AddEntry(xsec,"stat only","l");
  legxsec->AddEntry(xsec_withsyst,"stat + syst","l");
...
void plot_datasets_axis1(std::vector<RooDataSet*> dset, TString outname, legend_struct legdata){
  ...
  Int_t colors[4] = {kBlack,kRed,kBlue,kGreen};
  ...
  for (int j=0; j<ndsets; j++) leg->AddEntry(h[j],legdata.leg[j].Data());
  leg->Draw();
